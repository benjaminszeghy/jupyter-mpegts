...
```bash
python -m py_compile jupyter_mpegts/__init__.py  # Check single file for syntax errors
```
...
```typescript
const PLUGIN_ID = 'jupyter_mpegts:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyter_mpegts:open-panel';
  export const refreshData = 'jupyter_mpegts:refresh-data';
}
...
- **Output**: Creates files in `lib/` and `jupyter_mpegts/labextension/`
...
- Ask user to search for the extension ID (`jupyter_mpegts`) to see if it loaded
...
ls -la jupyter_mpegts/labextension/  # Should contain bundled extension
```
...
- **Plugin ID** (in `src/index.ts`): `'jupyter_mpegts:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyter_mpegts:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyter-mpegts-ClassName`
